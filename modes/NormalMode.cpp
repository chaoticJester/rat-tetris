...
void NormalMode()
{
    ...
    GameState state = GameState::JUSTSTART;
    ...
    while (!WindowShouldClose())
    {
        ...
        else
        {
            ...
            if (state == GameState::PLAYING)
            {
                ...
                else
                {
                    ...
                    {
                        if (game.rotate(Direction::CW))
                            actionSuccess = true;
                    }
                    if (IsKeyPressed(KEY_Z))
                    {
                        if (game.rotate(Direction::CCW))
                            actionSuccess = true;
                    }
        ...
        if (state == GameState::JUSTSTART)
        {
            ...
            DrawText(TextFormat("%.0f", startTimer), 600, 300, 100, GOLD);
        }
        else if (state == GameState::TRANSITION)
        {
